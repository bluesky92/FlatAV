...
BOOL CArchiveOp::IsArchive(LPCTSTR szFilePathName)
{
	...
	//_tsopen_s(&handle, szFilePathName, _O_RDONLY | _O_BINARY, _SH_DENYNO, _S_IREAD | _S_IWRITE);
			...
			if (fTemp == NULL) {
				...
				if (zerr == UNZ_OK) {
					...
					if (ret == FL_VIRUS || ret == FL_SUSPECT) {
						...
						_tcscpy_s(pMWObj->szFileNameInArchive, _countof(pMWObj->szFileNameInArchive), szU);
#else
						_tcscpy_s (pMWObj->szFileNameInArchive, _countof(pMWObj->szFileNameInArchive), szFileNameInZip);
#endif
						_tcscpy_s(pMWObj->szFilePathName, _countof(pMWObj->szFilePathName), szZipFPN);
