...
void ShowHelp()
{
	_tprintf(_TEXT("FvdTool\n"));
	_tprintf(_TEXT("Usage: FvdTool <Option> [List_of_Arguments]\n\n"));
	_tprintf(_TEXT("Option:\n"));
	_tprintf(_TEXT("  -b: Build new database.\n"));
	_tprintf(_TEXT("      Args: [output_dbfilename] [input_dir] [input_filename]\n"));
	_tprintf(_TEXT("      Default values:\n"));
	_tprintf(_TEXT("        output_dbfilename = main.fvd\n"));
	_tprintf(_TEXT("        input_dir = <current directory>\n"));
	_tprintf(_TEXT("        input_filename = main\n"));
	_tprintf(_TEXT("  -c: Validate database.\n"));
	_tprintf(_TEXT("      Args: dbfilename [Minimum_AV_Major_Version] [Minimum_AV_Minor_Version]\n"));
...
int ValidateDb(_TCHAR* dbname, unsigned short wCurMinAvMajorVersion,
	unsigned char bCurMinAvMinorVersion)
{
	...
	nRet = Fvd_ValidateDb(&fvdBlock, wCurMinAvMajorVersion, bCurMinAvMinorVersion);
	if (nRet != 0)
		return nRet;
			...
			switch (nAction)
			{
				case ACT_BUILDNEWDB:
				{
					switch (nArgCount)
					{
						...
						case 3: default: nRet = BuildNewDb(argv[i], argv[i + 1], argv[i + 2]); break;
					}
				}
				break;
				...
				case ACT_VALIDATEDB:
				{
					...
					case 3: default: nRet = ValidateDb(argv[i], _ttoi(argv[i + 1]), _ttoi(argv[i + 2])); break;
					}
				}
				break;
