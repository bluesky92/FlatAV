...
static int SDBScan(int handle, LPFVD_SDBREC lpSdbRec, int iFileType, LPPE_HEADER lpPeHdr, LPFVD_BLOCK lpFvdBlock)
{
	...
	if (dwBuffSize > 0) {
		...
		if (ParseSignHexStr(lpSdbRec->szSignHexStr, lpSdbRec->dwRecSize - FVD_MIN_SDBREC_SIZE, &lstSignHexByte) == -1) {
...
static int XDBScan(int handle, LPFVD_XDBREC lpXdbRec, LPPE_HEADER lpPeHdr, LPFVD_BLOCK lpFvdBlock)
{
	...
	md5Buffer(buffer, lpXdbRec->dwSignLength, md5Hash, MD5LEN);

	if (memcmp(md5Hash, lpXdbRec->md5Hash, MD5LEN) == 0) {
...
int Sca_ScanFile(const _TCHAR* szFilePathName, /*LPSCA_OPT lpScaOpt,*/ LPFVD_BLOCK lpFvdBlock)
{
	...
	_tsopen_s(&handle, szFilePathName, _O_RDONLY | _O_BINARY, _SH_DENYWR, _S_IREAD);
	...
	memset(fvdFindCwdbRec.md5Hash, 0, MD5LEN);

	/* Get md5 hash from the file */
	_lseek(handle, 0, SEEK_SET);
	md5File(handle, fvdFindCwdbRec.dwFileSize, fvdFindCwdbRec.md5Hash, MD5LEN);
	...
	if (aSdbRec != NULL && dwSdbRecCount != 0) {
		for (i = 0; i < dwSdbRecCount; ++i) {
			...
			if (ret != FL_CLEAN)
				goto LReturn;
			(char*)aSdbRec += aSdbRec->dwRecSize;
		}
	}
